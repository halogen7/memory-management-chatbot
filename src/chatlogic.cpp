...
ChatLogic::ChatLogic()
{
    ...
    // Removed new chatbot initialization in constructor
...
ChatLogic::~ChatLogic()
{
    ...
    // Removed explicit deallocation handling
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // Get raw pointer since parent nodes not owned
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                rootNode = (*it).get(); // assign current node to root; get raw pointer to node
            }
            else
            {
